...
namespace DataStructures.Trees
{
    ...
    public class AVLTree<T> : BinarySearchTree<T> where T : IComparable<T>
    {
        ...
        /// <returns></returns>
        protected override BSTNode<T> _insertImpl(T item)
        {
            // New node object
        ...
        public override bool Remove(T item)
        {
            ...
            var node = (AVLTreeNode<T>)_findNode(Root, item);
        ...
        /// </summary>
        public override bool RemoveMin()
        {
            if (IsEmpty)
                return false;

            // Get the node from the tree
            var node = (AVLTreeNode<T>)_findMinNode(Root);

            // Invoke the BST remove node method.
            var state = _remove(node);
            if (state)
            {
                // Rebalance the tree
                // node.parent is actually the old parent of the node,
        ...
        /// </summary>
        public override bool RemoveMax()
        {
            if (IsEmpty)
                return false;

            // Get the node from the tree
            var node = (AVLTreeNode<T>)_findMaxNode(Root);
