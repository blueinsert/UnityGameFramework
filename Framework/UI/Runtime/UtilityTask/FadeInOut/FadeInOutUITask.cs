...
namespace bluebean.UGFramework.UI
{
    ...
    public class FadeInOutUITask : UITaskBase
    {
        public FadeInOutUITask(string name) : base(typeof(FadeInOutUITask).Name)
        {
        }
        ...
        public static void StartShowFadeOutLoadingFadeIn(Action fadeOutEnd, FadeStyle style = FadeStyle.Black, float fadeOutTime = -1, float fadeInTime = -1)
        {
            PostTaskAction((task) => { 
                task.FadeInOutUIController.StartShowFadeOutLoadingFadeIn(fadeOutEnd,style,fadeOutTime,fadeInTime);
            });
        }

        public static void HideFadeOutLoadingFadeIn()
        {
            PostTaskAction((task) => {
                task.FadeInOutUIController.HideFadeOutLoadingFadeIn();
            });
        }
