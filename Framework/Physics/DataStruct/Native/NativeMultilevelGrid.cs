...
namespace bluebean.UGFramework.DataStruct.Native
{
    ...
    public unsafe struct NativeMultilevelGrid<T> : IDisposable where T : unmanaged, IEquatable<T>
    {
        ...
        public Unity.Collections.NativeList<Cell<T>> usedCells;
        ...
        public NativeMultilevelGrid(int capacity, Allocator label)
        {
            ...
            usedCells = new Unity.Collections.NativeList<Cell<T>>(label);
        ...
        public static void GetCellCoordsForBoundsAtLevel(Unity.Collections.NativeList<int4> coords, BurstAabb bounds, int level, int maxSize = 10)
        {
            coords.Clear();
